...
export const BookingProvider = ({ children }) => {
  ...
  const [selectedUser, setSelectedUser] = useState(null); // Selected time slot for the modal
  ...
  //handleAdminMonth below

  const handleAdminMonth = async (timeSlot, fetchBookings, unavailableTimes = []) => {
    console.log("calling AdminMonth");
    debugger;
  
    const walkerId = '66c63ca7a283afffb1194d38'; // Replace with actual walker ID
    const ownerId = selectedUser;
    ...
    for (const day of monthDates) {
      const unavailable = unavailableTimes.some((unavailableTime) => {
        ...
        const [time, modifier] = unavailableTime.time.split(" ");
      ...
      const newWalk = {
        ...
        endTime: new Date(day.getTime() + 60 * 60 * 1000).toISOString(),
      ...
      try {
        ...
        if (!walkResponse.ok) throw new Error("Failed to create walk");
        ...
        const paymentData = {
          amount: selectedWalkTime === "30" ? 25.0 : 50.0,
        ...
        if (!updateBookingResponse.ok) throw new Error("Failed to update booking status");
  ...
  //handleAdminWeek below 

  const handleAdminWeek = async (timeSlot, fetchBookings, unavailableTimes = []) => {
    console.log("calling AdminWeek");
  
    const walkerId = '66c63ca7a283afffb1194d38'; // Replace with actual walker ID
    const ownerId = selectedUser; // Use selectedUser instead of authState.user.id
  ...
  // handleAdminWalk below 

  const handleAdminWalk = async (timeSlot, fetchBookings) => {
    if (!selectedDog) {
    ...
    console.log("Calling handleAdminWalk");
  
    const walkerId = '66c63ca7a283afffb1194d38'; // Replace with actual walker ID
    const ownerId = selectedUser; // Use selectedUser as ownerId
    ...
    try {
      ...
      const bookingData = {
        walkerId,
        ownerId, // Use selectedUser's _id as ownerId
        dogId: selectedDog,
        walkId,
        status: 'pending',
      };
      ...
      console.error('Error creating admin booking and payment:', err);
      alert('Failed to book walk and process payment');
    } finally {
      // Reset the loading state for the selected time slot
      setLoadingSlots((prev) => ({
        ...prev,
  ...
  return (
    <BookingContext.Provider value={{ selectedUser, setSelectedUser,selectedDog, setSelectedDog, selectedWalkTime, setSelectedWalkTime, handleBookWalk, handleBookWeek, handleBookMonth, handleAdminMonth, handleAdminWeek, handleAdminWalk, cancelBooking, loadingSlots, loadingWeekSlots, loadingMonthSlots }}>
      {children}
